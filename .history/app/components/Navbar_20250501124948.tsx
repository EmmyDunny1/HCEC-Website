...
const navItems: NavItem[] = [
  ...
  { label: "CONTACT US", link: "#", children: [{ label: "Our History", link: "#" }] },
];
          ...
          <nav className="text-[10px] hidden md:flex justify-between items-center gap-10">
            {navItems.map((d, i) => (
              ...
              >
                ...
                {/* Dropdown */}
                {d.children && activeDropdown === i && (
                  <div className="dropdown cursor-pointer absolute flex flex-col px-[2rem] top-10 bg-amber-50 text-black w-[13rem] transition-all text-[12px] border-t-4 border-red-500 outline-t-red-500">
                    {d.children.map((ch, j) => (
                      <a
                        key={j}
                        href={ch.link ?? "#"}
