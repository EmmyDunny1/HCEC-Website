...
export default function Navbar({
  ...
  return (
    <div className="bg-[#0C101C]">
      <div className="fixed top-0  bg-red-800 flex max-w-7xl w-full mx-auto text-white p-4 transition-all border-b-1  shadow-2xl hover:border-0 items-center z-50">
  ...
  Evangelical{churchName.split("Evangelical")[1] || ""}
        ...
        <button onClick={toggleMenu}>{
          menuOpen ? (< BiMenuAltRight className="text-2xl" />) : (< RiCloseLine className="text-2xl" />)
          }</button>
          </div>
        {/* mobile menu dropdown */}
         {menuOpen && (
          <div className="absolute top- left-0 w-full bg-blue-950 text-white shadow-lg items-center z-50">
            <ul className="flex flex-col items-center p-4">
              {navItems.map((item, index) => (
                <li key={index} className="w-full text-center mb-2">
                  ...
                  {item.children && (
                    <ul className="bg-gray-100 mt-2 rounded-lg shadow-md">
