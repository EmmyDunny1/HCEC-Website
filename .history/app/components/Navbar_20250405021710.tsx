...
<section className='flex transition-all justify-between items-center gap-20'>
    ...
    <nav className='fixed text-[10px] font-semibold flex justify-between items-center gap-10'>
        {navItems.map((d, i) => (
        <Link 
        key={i}
        href={ d.link ?? "#"} className='group items-center transition-all relative'>
        <p className=' flex cursor-pointer hover:border-b-2 pb-5 border-pink-400'>
            <span className=''>{d.label}</span>
            {/* dropdown */}
             </p>
        <div className='flex-col px-[2rem]  justify-items-start absolute hidden justify top-10 right-0 left-0 group-hover:flex  bg-amber-50 text-black w-[13rem] transition-all text-[12px]  hover:border-pink-400'>
                   {d.children?.map((ch, i) => (
