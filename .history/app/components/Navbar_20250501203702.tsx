          ...
          <nav className="text-[10px] hidden md:flex justify-between items-center gap-10">
            {navItems.map((d, i) => (
              <div key={i} className="group relative">
                ...
                {/* Dropdown */}
                {d.children && (
  <div className="absolute flex fle top-10 bg-amber-50 text-black w-[13rem] transition-all text-[12px] border-t-4 border-red-500 opacity-0 invisible group-hover:opacity-100 group-hover:visible">
    {d.children.map((ch, j) => (
