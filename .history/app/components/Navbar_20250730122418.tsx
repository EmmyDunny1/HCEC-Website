...
export default function Navbar({
  ...
  Evangelical{churchName.split("Evangelical")[1] || ""}
        ...
        {/* mobile menu dropdown */}
         {menuOpen && (
          <div className="absolute top-16 left-0 w-full bg-[#0C101C] text-white shadow-lg  z-50">
            <ul className="flex flex-col items-center p-4">
              {navItems.map((item, index) => (
                <li key={index} className="w-full text-center mb-2">
                  <Link href={item.link ?? "#"} className="block py-2 px-4 hover:bg-red-500 hover:text-[#0C101C]">
                    {item.label}
                  </Link>
                  {item.children && (
