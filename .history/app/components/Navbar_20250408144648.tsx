...
export default function Navbar() {
  return (
    ...
    <div className='flex max-w-7xl w-full mx-auto  text-white p-4 transition-all border-b-1 rounded-b-2xl  shadow-2xl border-red-500  hover:border-0 '>
        ...
        <nav className='text-[10px] flex justify-between items-center gap-10'>
          {navItems.map((d, i) => (
            <div key={i} className='group relative'>
              ...
              {/* Dropdown */}
              {d.children && (
                <div className='dropdown cursor-pointer absolute hidden group-hover:flex flex-col px-[2rem] top-10 bg-amber-50 text-black w-[13rem] transition-all text-[12px] border-t-2 border-pink-400'>
                  {d.children.map((ch, j) => (
                    <a key={j} href={ch.link ?? "#"} className='py-2 px-4 hover:text-red-400'>
                      {ch.label}
                    </a>
                  ))}
                </div>
