...
<section className='flex transition-all justify-between items-center gap-20'>
    ...
    <nav className=' text-[10px] font-semibold flex justify-between items-center gap-10'>
        ...
        <div className='flex-col px-[2rem]  justify-items-start absolute hidden justify top-10 right-0 left-0 group-hover:flex  bg-amber-50 text-black w-[13rem] transition-all text-[12px] '>
                   {d.children?.map((ch, i) => (
                <a key={i} href={ch.link?? "#"} className='py-2 px-4 hover:text-pink-400 border-t-2 hover:border-pink-400'>
                   {ch.label}
                   </a>
                   ))}
